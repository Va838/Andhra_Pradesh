...
exports.AndhraCulturalGuide = void 0;
const components_1 = require("./components");
const ResponseFormatter_1 = require("./components/ResponseFormatter");
 ...
 */
class AndhraCulturalGuide {
    constructor() {
        // Initialize all components
        this.culturalDatabase = new data_1.CulturalDatabaseImpl();
        this.dropdownInterface = new components_1.DropdownInterfaceImpl();
        this.inputProcessor = new components_1.InputProcessorImpl();
        this.responseGenerator = new components_1.ResponseGeneratorImpl(this.culturalDatabase);
        this.responseFormatter = new ResponseFormatter_1.ResponseFormatter();
    }
    /**
     ...
     */
    async getCulturalGuidance(category, selection, preferences) {
        try {
            ...
            const formattedError = this.responseFormatter.formatErrorResponse(errorMessage, category);
            return {
     ...
     */
    getErrorMessage(category, selection, preferences) {
     ...
     */
    getSuggestions(category, failedSelection) {
        try {
            const subcategories = this.getSubcategories(category);
            // If a specific selection failed, suggest similar options
            if (failedSelection) {
                const lowerFailed = failedSelection.toLowerCase();
                const similar = subcategories.filter(sub => sub.toLowerCase().includes(lowerFailed) ||
     ...
     */
    formatCustomResponse(content, category, region) {
        const mockResponse = {
            content,
     ...
     */
    generateUserFriendlyError(category, selection, error) {
...
exports.AndhraCulturalGuide = AndhraCulturalGuide;
//# sourceMappingURL=AndhraCulturalGuide.js.map